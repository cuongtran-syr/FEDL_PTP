...
def load_public_airport_features(label_pd, airport_name):
    ...
    rw_file_name = PUBLIC_DATA_PATH.format(airport_name, airport_name, airport_name, 'runways')
    runways = pd.read_csv(rw_file_name, parse_dates=["timestamp"])

    weather_file_name = PUBLIC_DATA_PATH.format(airport_name, airport_name, airport_name, 'lamp')
    lamp = pd.read_csv(weather_file_name,
                       parse_dates=["forecast_timestamp"]).drop_duplicates(subset='forecast_timestamp')
...
def load_private_airport_airline_features(label_pd, airport_name, airline):
    mfs_file_name = PRIVATE_DATA_PATH.format(airport_name, airport_name, airport_name, airline)
...
def extract_all_airlines_data():
    ...
    for airport_name in AIRPORTS:
        label_pd = pd.read_csv(LABEL_PATH.format(airport_name, airport_name), parse_dates=["timestamp"])
        label_pd["airline_name"] = label_pd["gufi"].str[:3]
        label_pd = label_pd[label_pd['airline_name'].isin(AIRLINES)]
    ...
    for airport_name in AIRPORTS:
        ...
        for airline in AIRLINES:
            sub_label_pd = label_pd[label_pd['airline_name']==airline][['gufi','timestamp']]
            priv_pd, priv_features = load_private_airport_airline_features(sub_label_pd, airport_name, airline)
