...
def get_basic_feats(test_pd):
    ...
    for val in ['early_morning', 'morning', 'noon', 'afternoon', 'night']:
        test_pd['block_time_in_{}'.format(val)]  = test_pd['block_time'].apply(lambda x: x==val).astype(int)
        features.append('block_time_in_{}'.format(val))

    for val in ['monday','tuesday','wednesday','thursday','friday','saturday','sunday']:
        test_pd['day_is_{}'.format(val)] = test_pd['day_in_wk'].apply(lambda x: x == val).astype(int)
...
def get_weather_info(test_pd, lamp, airport_name):
    ...
    final_pd = pd.merge_asof(final_pd.sort_values(by='timestamp'), weather_pd.sort_values(by='timestamp'),
                             on='timestamp', tolerance=pd.Timedelta("30h"), direction='backward')
    features = ['wind_speed', 'wind_direction', 'temperature', 'wind_gust', 'cloud_ceiling', 'visibility', 'precip']
    final_pd['precip'] = final_pd['precip'].apply(lambda x: x== True).astype(int)
    cloud_vals = ['OV', 'CL', 'FW', 'BK', 'SC']
    for val in cloud_vals:
        final_pd['cloud_is_{}'.format(val)] = final_pd['cloud'].apply(lambda x: x==val).astype(int)
        features += ['cloud_is_{}'.format(val)]

    lightning_prob_vals = ['N', 'L', 'M', 'H']
    for val in lightning_prob_vals:
       final_pd['lightning_prob_is_{}'.format(val)] = final_pd['lightning_prob'].apply(lambda x: x == val).astype(int)
...
def get_meta_features(mfs):
    ...
    """
    TOP_AIRCRAFT_TYPES = ['B738', 'A321', 'A320', 'B739', 'CRJ9', 'B737', 'A319', 'E75L', 'CRJ7',
                          'CRJ2', 'E170', 'E145', 'B752', 'B763', 'B712', 'B38M', 'A20N', 'E75S',
                          'A21N', 'DH8D', 'A306', 'MD11', 'B772', 'B39M', 'E190']
    ...
    for aircraft in TOP_AIRCRAFT_TYPES:
        mfs['is_aircraft_{}'.format(aircraft)] = mfs['aircraft_type'].apply(lambda x: x == aircraft).astype(int)
        features.append('is_aircraft_{}'.format(aircraft))

    for carrier in TOP_MAJOR_CARRIERS:
        mfs['is_carrier_{}'.format(carrier)] = mfs['major_carrier'].apply(lambda x: x == carrier).astype(int)
        features.append('is_carrier_{}'.format(carrier))

    for flight_type in FLIGHT_TYPES:
        mfs['is_flight_type_{}'.format(flight_type)] = mfs['flight_type'].apply(lambda x: x == flight_type).astype(int)
...
def get_diff_depart_vs_ts(test_pd,  etd):
    ...
    """
    etd = etd.copy()
    final_pd = pd.merge_asof(test_pd.sort_values(by=['timestamp']), etd.sort_values(by=['timestamp']),
                             on='timestamp', by='gufi', tolerance=pd.Timedelta("30h"), direction='backward')
    final_pd['diff_depart_vs_ts'] = pd.to_timedelta(final_pd['departure_runway_estimated_time'] - final_pd['timestamp'],
                                                    unit='m')
    final_pd['diff_depart_vs_ts'] = final_pd['diff_depart_vs_ts'].astype('timedelta64[m]')
...
def get_num_dep_flights(test_pd, runways):
    ...
    rw['prev_date_hour'] = rw['prev_timestamp_1hr'].apply(lambda x: str(x).split(":")[0])
