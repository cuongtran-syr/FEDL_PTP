...
if __name__ == "__main__":
    ...
    for airport_name in AIRPORTS:
        for airline in AIRLINES:
            partial_submission_format = submission_format[ submission_format['airline_name']==airline]
            priv_pd, priv_features = load_private_airport_airline_features(partial_submission_format, airport_name, airline)
            private_pd_list.append(priv_pd)

    private_pd = pd.concat(private_pd_list)
    pd_all = pd.merge(public_pd, private_pd, on=['gufi', 'timestamp','airport','airline_name'], how='left')
    pd_all = pd_all.fillna(0)
    temp_pd = pd.merge(submission_format, pd_all, on = ['gufi', 'timestamp','airport','airline_name'], how ='left' )
    ...
    submission_format.to_csv('./processed/submission.csv')
